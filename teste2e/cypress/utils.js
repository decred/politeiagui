...
export const typeDatePicker = (testID, timestamp) => {
  cy.findByTestId(testID).children().first().click();
  cy.wait(100);
    cy.findByTestId(testID).children().first() // input here
        .children().eq(1) //rmpContainer
        .children().eq(1) //rmpCell
        .children().first() //rmpPopup
        .children().first().as("datepickerPad"); //rmpPad
  const date = new Date(timestamp * 1000);
  cy.get("@datepickerPad").children().first().children().first().then(($el) => {
    const year = Number($el.text());
    const targetYear = date.getFullYear();
    cy.log(year, targetYear);
    if (year !== targetYear) {
      const dataId = year < targetYear ? ">" : "<";
      const loopTimes = Math.abs(targetYear - year);
      for (let i = 0; i < loopTimes; i++) {
        cy.wait(200);
        cy.get("@datepickerPad").children().first().contains(dataId).click();
      }
    }
  });
  cy.get("@datepickerPad").children().eq(1).children().first().then(($el) => {
    const monthLabel = $el.text();
    const month = monthLabels.indexOf(monthLabel);
    const targetMonth = date.getMonth();
    cy.log(month, targetMonth);
    if (month !== targetMonth) {
      const dataId = month < targetMonth ? ">" : "<";
      const loopTimes = Math.abs(targetMonth - month);
      for (let i = 0; i < loopTimes; i++) {
        cy.wait(200);
        cy.get("@datepickerPad").children().eq(1).contains(dataId).click();
      }
    }
  });
  const dateNum = date.getDate();
  cy.get("@datepickerPad").children().eq(2).children().eq(dateNum - 1).click();
};
