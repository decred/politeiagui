      ...
      <div className="detail-form">
        ...
        <p>
          {showIdentityHelpText ? (
            <div>
              <span style={{ fontWeight: "bold", maxWidth: "7em", marginLeft: "11.7em" }}>{this.identityHelpPrompt}</span>{" "}
              <a className="linkish" onClick={() => this.setState({ showIdentityHelpText: false })}>
              (hide)
              </a>
            </div>
          ) : (
            <a className="linkish" style={{ maxWidth: "7em", marginLeft: "11.7em" }} onClick={() => this.setState({ showIdentityHelpText: true })}>
        ...
        <Field label="Password"><a className="linkish" onClick={() => openModal(CHANGE_PASSWORD_MODAL)}>Change Password</a></Field>
        <FieldSeparator />
