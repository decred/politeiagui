...
// Curently fetching user's proposals isn't available on the BE side
// and will be added in later.
export function useUserProposals(/*userID*/) {
  ...
  //      }
  //      if (!isEmpty(rfpLinks) && !isEmpty(unfetchedLinks)) {
  //        onFetchProposalsBatch(unfetchedLinks, false) // XXX should provide newly added params
  //          .then(() => send(VERIFY))
  //          .catch((err) => send(REJECT, err));
  //        return send(FETCH);
  //      }
