...
describe("test external_api selector", () => {
  ...
  test("testing higher order selectors", () => {
    // isRequesting
    expect(sel.getIsExtApiRequesting("payWithFaucet")(MOCK_STATE))
      .toEqual(MOCK_STATE.external_api.payWithFaucet.isRequesting);

    // response
    expect(sel.getExtApiResponse("payWithFaucet")(MOCK_STATE))
      .toEqual(MOCK_STATE.external_api.payWithFaucet.response);

    // error
    expect(sel.getExtApiError("payWithFaucet")(MOCK_STATE))
      .toEqual(MOCK_STATE.external_api.payWithFaucet.error);
  ...
  test("testing 'or' and composed selectors", () => {
    ...
    state = { external_api: { ...MOCK_STATE.external_api, payWithFaucet: { isRequesting: true } } };

    expect(sel.isApiRequestingPayWithFaucet(state))
      .toEqual(true);

    state = { external_api: { ...MOCK_STATE.external_api,
